...
import { DomainError, IllegalMatchStateError, MatchNotFoundError } from "../domain/error";
...
const scenarios: CommandScenario<PlayMoveCommand>[] = [
  ...
  },
  {
    ...
    handleCommand: playMoveCommandHandler(mock({ matchToFind: challengeMatch })),
    command: new PlayMoveCommand({ matchId, move: [bob, 1] }),
    expected: failure([new IllegalMatchStateError(challengeMatch, ["New", "Game"])]),
  },
  {
  ...
  },
  {
    ...
    command: new PlayMoveCommand({ matchId, move: [bob, 1] }),
    expected: failure([new IllegalMatchStateError(victoryMatch, ["New", "Game"])]),
  },
  {
    name: "create challenge",
    handleCommand: playMoveCommandHandler(mock({ matchToFind: newMatch })),
    command: new PlayMoveCommand({ matchId, move: [alice, 0] }),
    expected: success(new CreateChallenge({ matchDescription, move: [alice, 0] })),
  },
  {
    name: "play move",
